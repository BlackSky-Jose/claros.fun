...
const CentralBranding: React.FC<CentralBrandingProps> = ({ isCleaning, isPreparing = false, onCleanClick }) => {
  return (
    <div className=" relative z-10 flex flex-col items-center justify-center h-full px-4 mb-10 pb-10 mt-20 ">
      <div onClick={() => window.open('https://x.com/Clarosdotfun', '_blank')} className = "absolute top-2 left-2 border border-[#B476F2] rounded-md cursor-pointer !px-2 py-2 !flex items-center gap-2 text-[#B476F2] bg-black/80 backdrop-blur-sm font-bold hover:bg-black/90 hover:text-[#B476F2] transition-all duration-200 animate-faint-pulse" >
